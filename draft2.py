...
def fit_gmm(num_components, beta, num_step=20, num_samples=1):
    ...
    for i in range(num_step):
        ...
        d2xx = 0
        objective_value = 0
        sampled_z, ixs = gmm_sample(objective.q_means, objective.q_stds, objective.q_log_pais, num_samples)
        ...
        for c in range(num_components):
            ...
            delta = gaussian(sampled_z, objective.q_means[c], objective.q_stds[c]) / torch.exp(objective.q_fn(sampled_z)).T
        ...
        for c in range(num_components):
            ...
            change =  .1*beta * (all_deltas[c] - all_deltas[-1]) * objective_value
            rho[c] = .99*rho[c] - change
            print(ixs)
...
if __name__ == '__main__':
    all_normalized_posteriors, normalized_true_posterior, x_axis = fit_gmm(num_step=201, num_components=2, beta=0.1, num_samples=1)
